...
@pytest.mark.asyncio
async def test_llm_router_and_client_simulation():
    ...
    response = await planner_client.completion(messages)
...
async def test_planner_agent():
    ...
    plan = await planner.create_plan("Find a high-strength steel formulation")
    assert len(plan) > 0
    assert any("encode" in step.lower() or "classical" in step.lower() for step in plan)

@pytest.mark.asyncio
async def test_interpreter_agent():
    router = LLMRouter(config_path="config/llm_routing.yaml")
    ...
    findings = await interpreter.interpret(execution_summary, "Grover")
...
async def test_judge_agent():
    ...
    report = await judge.evaluate("Find a COX-2 inhibitor", interpretation)
